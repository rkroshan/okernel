 ...
 * 
 */
static void buddy_toggle_bitmap(uint64_t index, uint8_t order, FreeArea_t* area)
{
 ...
 * 
 */
static uint8_t buddy_get_bitmap(uint64_t index, uint8_t order, FreeArea_t* area)
{
  /*block_index in that order = index of page / (order)
  pair_indx = block_idx/2
  bitmap_idx = BITMAP_INDEX(pair_indx)
  bit_position = BIT_POSITION(pair_indx)*/
  uint64_t pair_indx = (index/BIT(1+order));
  return (uint8_t)((area->bitmap[BITMAP_INDEX(pair_indx)] & (1UL << BIT_POSITION(pair_indx))) >> BIT_POSITION(pair_indx));
}
...
void buddy_heap_init() {
  ...
  for(uint8_t zone_idx=0;zone_idx < ZONES_COUNT;zone_idx++)
  {
    ...
    printk("zone_start_addr : %x size : %x\n", zone->start_addr, zone->size);
    /*memory region should be PAGE_SIZE aligned else initialisation will fail with panic*/
    ...
    for(uint8_t order=MAX_ORDER -1;order>=0; order--)
    {
      ...
      printk("buddy_heap_init: order:%d nblocks:%u new_mem_start:%x size_remaining:%x\n", order, nblocks, available_memory_start, available_memory_size);
    }
  }
}
...
static page_t *buddy_alloc(uint8_t order) {
  ...
  for(uint8_t zone_idx=0;zone_idx < ZONES_COUNT;zone_idx++)
  {
    ...
    for (uint8_t current_order = order; current_order < MAX_ORDER;
      ...
      if (zone->area[current_order].freeblocks_list != NULL) {
        ...
        FreeBlock_t *block =
            zone->area[current_order].freeblocks_list;
        ...
        if bitmap indx remain 1 while freeing it means other buddy is still in freearea list we can merge*/
        buddy_toggle_bitmap(get_page_indx((uint64_t)block),current_order,&zone->area[current_order]);
        ...
        while (current_order > order) {
          ...
          uint64_t buddy_address = (uint64_t)block ^ (1UL << current_order);
          ...
          buddy_toggle_bitmap(get_page_indx(buddy_address),current_order,&zone->area[current_order]);
        }
  ...
  printk("Buddy_alloc: No block found!!!\n");
...
static void buddy_free(page_t* page, uint8_t order) {
  ...
  zone_t* zone = get_zone_info(page->zone_id); /*zone in which page is present*/
    ...
    assert((address >= zone->start_addr) && (address < (zone->start_addr + zone->size)));
    ...
    while (current_order < MAX_ORDER) {
      ...
      uint8_t bitset = buddy_get_bitmap(get_page_indx(address),current_order,&zone->area[current_order]) & 1U;
      /*update the bitmap*/
      buddy_toggle_bitmap(get_page_indx(address),current_order,&zone->area[current_order]);
      if (bitset) {
        /*that means another block is free we can merge this block with that*/
        /*remove the buddy from freeblock list of current order*/
        FreeBlock_t *current_block =
            zone->area[current_order].freeblocks_list;
        ...
        if ((uint64_t)buddy != buddy_address) {
          ...
          printk("BUDDY_FREE: This should not happen!!");
        }

        /*if second block was free and first was already then start address should be first block address
        it is necessary because that's how we will be able to find the buddy in next order*/
