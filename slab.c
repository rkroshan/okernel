...
static slab_t* alloc_slab(kmem_cache_t* cache)
{
    ...
    */
    uint64_t buf_clt_array_size = (((uint64_t)slab + get_page_size()) - buf_ctl_start)/(cache->objsize + sizeof(kmem_bufctl_t));
    /*so smem from where actual object allocation will start will be after kmem_buf_ctl*/
    slab->smem = (buf_ctl_start + buf_clt_array_size*sizeof(kmem_bufctl_t));
    /*now need to initialise the kmem_buf_ctl*/
    initialize_kmem_buf_ctl((kmem_bufctl_t*)slab_bufctl(slab), buf_clt_array_size);
...
static void* alloc_slab_partial_mem(kmem_cache_t* cache)
{
    ...
    void* addr =  (void*)(current_partial_slab->smem*current_partial_slab->free);
 ...
 * - first it will check in partial_slab, if no partial slab put an empty slab into partial
 * and allocate the memory, also update the buf_ctl
...
static void* alloc_slab_mem(kmem_cache_t* cache)
{
    ...
    if(cache->slabs_partial != NULL)
    {
        ...
        {
            cache->slabs_empty = alloc_slab(cache->objsize);
        }

        /*put the empty_slab in partial slab*/
        kmem_cache_t* empty_slab = cache->slabs_empty;
...
void kmem_cache_free(void* ptr, page_t* page)
{
    ...
    slab_t* slab = (uint64_t)page->start_addr;
