...
FORMAT_PY_FILE = ".format-py-stamp"
# python_files list
python_files_list = []
...
def generate_ninja_build(source_files, build_dir="build"):
    ...
    for source_file in source_files:
        ...
        writer.build(
            ...
            rule="cc",
            inputs=[source_file],
            order_only=[FORMAT_FILE, FORMAT_PY_FILE],
        )
        writer.newline()
    ...
    writer.build(
        ...
        rule="link",
        inputs=object_files,
        order_only=[PREPROCESSED_LINKER],
    )
    writer.newline()

    # objdump the kernel elf
    writer.build(
        os.path.join(build_dir, OBJDUMP_NAME),
        rule="objdump",
        inputs=[os.path.join(build_dir, ELF_NAME)],
        order_only=[os.path.join(build_dir, ELF_NAME)],
    )
    # dump the kernel elf information
    writer.build(
        os.path.join(build_dir, ELFDUMP_NAME),
        rule="readelf",
        inputs=[os.path.join(build_dir, ELF_NAME)],
        order_only=[os.path.join(build_dir, ELF_NAME)],
    )
    ...
    writer.build(
        ...
        rule="gen_docs",
        inputs=None,
        order_only=[os.path.join(build_dir, OBJDUMP_NAME)],
    )
...
def get_target_info():
    parser = argparse.ArgumentParser(
        description="generate rule and build command commands for ninja"
    )
...
python_files_list = find_files_with_extns(WORKSPACE_DIR, extn=".py")
